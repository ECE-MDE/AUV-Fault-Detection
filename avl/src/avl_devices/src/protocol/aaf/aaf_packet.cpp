...
//------------------------------------------------------------------------------
std::vector<AafPacket> AafPacket::parse_multiple(std::vector<uint8_t> bytes)
{

    // Vector of packets to return
    std::vector<AafPacket> packets;
    ...
    while(bytes.size() > 0)
    {
        ...
        std::vector<uint8_t> packet_bytes = avl::subvector(
            bytes, 0, packet_length);

        // Create a packet from the packet bytes and put it in the packet vector
        AafPacket packet(packet_bytes);
...
//------------------------------------------------------------------------------
AafPacket::AafPacket(std::vector<uint8_t> packet_bytes)
{
    set_bytes(packet_bytes);
}

//------------------------------------------------------------------------------
// Name:        AafPacket destructor
...
//------------------------------------------------------------------------------
std::vector<uint8_t> AafPacket::get_bytes()
{
...
//                and the checksum bytes.
//------------------------------------------------------------------------------
void AafPacket::set_bytes(std::vector<uint8_t> packet_bytes)
{
    ...
    if(payload_length > 0)
    {
        ...
        std::vector<uint8_t> payload_bytes = avl::subvector(
            packet_bytes, 4, payload_length);

        // The payload may contain multiple fields. The first field will be
        // parsed and then removed from the vector of payload bytes. The next
        ...
        try
        {
            ...
            while (payload_bytes.size() > 0)
            {
                ...
                // packet payload
                fields.push_back(AafField(avl::subvector(
                    payload_bytes, 0, field_length)));
        ...
        {
            throw std::runtime_error("set_bytes: failed to parse improperly "
                "formatted field bytes");
        }
...
//------------------------------------------------------------------------------
uint8_t AafPacket::get_descriptor()
{
    return descriptor;
}
...
//------------------------------------------------------------------------------
void AafPacket::set_descriptor(uint8_t packet_descriptor)
{
...
//------------------------------------------------------------------------------
bool AafPacket::has_field(uint8_t field_descriptor)
{
...
//------------------------------------------------------------------------------
int AafPacket::get_field_index(uint8_t field_descriptor)
{
...
//------------------------------------------------------------------------------
size_t AafPacket::get_num_fields()
{
    return fields.size();
}
...
// Returns:     field.
//------------------------------------------------------------------------------
AafField AafPacket::get_field(uint8_t field_descriptor)
{
...
// Arguments:   - field_descriptor: field descriptor byte
//------------------------------------------------------------------------------
void AafPacket::add_field(uint8_t field_descriptor)
{
    AafField field(field_descriptor);
...
//------------------------------------------------------------------------------
void AafPacket::add_field(uint8_t field_descriptor, std::vector<uint8_t> data)
{
    AafField field(field_descriptor, data);
    fields.push_back(field);
    payload_length += field.get_length();
}
...
//------------------------------------------------------------------------------
void AafPacket::add_field(AafField field)
{
    fields.push_back(field);
    payload_length += field.get_length();
}
...
//------------------------------------------------------------------------------
std::string AafPacket::get_string()
{
...
//------------------------------------------------------------------------------
std::vector<uint8_t> AafPacket::get_checksum(std::vector<uint8_t> bytes)
{
...
//------------------------------------------------------------------------------
void AafPacket::validate_bytes(std::vector<uint8_t> bytes)
{
